# 0 "fir.c"
# 1 "/home/ubuntu/caravel-soc_fpga-lab-main/lab-caravel_fir/testbench/counter_la_fir//"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "fir.c"
# 1 "fir.h" 1
...
void __attribute__ ( ( section ( ".mprjram" ) ) ) initfir() {
        ...
        (*(volatile uint32_t*)0x2600000c)=0xA50000;
 for(int i=0;i<=10;i++){
      (*(volatile uint32_t*)0x2600000c)=0xAB600000;
      (*(volatile uint32_t*)0x30000000) = taps[i];
  }
  (*(volatile uint32_t*)0x2600000c)=0xAB600000;
 (*(volatile uint32_t*)0x30000000) = 64;
  for(int n=0;n<64;n++){
    x[n]=n+1;
    (*(volatile uint32_t*)0x2600000c)=0xAB600000;
   (*(volatile uint32_t*)0x30000000) = x[n];
   while(1){
     if((*(volatile uint32_t*)0x30000000)==expect[n]){
       y[n]=expect[n];
  ...
  (*(volatile uint32_t*)0x2600000c)=(0x005A0000|(y[63]<<24))&0xFFFF0000;
}
